...
{def $form_data         = fetch( 'mwezforms', 'data', hash( 'form_id', $node.data_map.form.content.mwform_id ) )
     ...
     $attr_required     = NULL
     $has_ajax_access   = has_access_to_limitation( 'ezjscore', 'call', hash( 'FunctionList', 'mwezforms' ) )}

{* including CSS file *}
{ezcss_load( array( 'mwezforms.css' ) )}
...
<div class="mwform_container">
    ...
    {else}
        ...
        <form id="mwezform" method="POST" class="{$form_definition.css_class}" action={concat($node.url_alias, '#mwezform')|ezurl()}>

            <input type="hidden" name="mwform_id" value="{$form_definition.id}"/>
            <input type="hidden" name="node_id" value="{$node.node_id}"/>
            {foreach $form_attributes as $attribute}
                {set $attr_required = fetch( 'mwezforms', 'is_attrib_required', hash( 'attribute_id', $attribute.id ) )}
            ...
            {/foreach}
            <input id="mwezform-submit" type="submit" value="{'Send'|i18n( 'extension/mwezforms/front' )}"/>
